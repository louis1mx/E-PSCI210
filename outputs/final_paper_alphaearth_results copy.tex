...
\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{fig8_convlstm_residuals.png}
    \caption{Residual map. Positive values indicate observed cooling relative
    to the pre-policy reference.}
    \label{fig:convlstm_map}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{fig9_convlstm_timeseries.png}
    \caption{Predicted vs.\ observed spatial-mean LST (top) and training loss
    curves (bottom).}
    \label{fig:convlstm_ts}
  \end{subfigure}
  \caption{ConvLSTM post-policy evaluation. Left: spatial cooling residual map
  averaged over 2022--2025. Right: temporal performance and training diagnostics.}
  \label{fig:convlstm}
\end{figure}
...
The results also validate the feasibility of the three-layer detection approach.
The framework distinguishes greening from paving and identifies where thermal
and surface-change signals align, even at the coarse 780 m resolution imposed
by training constraints. Applying the same pipeline at 30 m or 10 m resolution,
aligned with park boundary data, would allow direct testing of whether
Chengdu's documented parks produced measurable local cooling.
