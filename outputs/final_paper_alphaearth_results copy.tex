...
cooling residuals in western and southwestern Chengdu. AlphaEarth embedding
...
policy date gives a clear before/after structure, but the empirical setting is
messy. Chengdu has strong seasonality, persistent cloud contamination, and
regional warming during the post-policy period. A simple comparison of average
temperature before and after 2022 is therefore not enough.
...
however, suggest localized cooling in western and southwestern Chengdu,
...
The first method is interrupted time series regression. This model estimates
whether there is a city-wide change in mean urban LST after January 2022. The
regression is:
\begin{equation}
...
The current ConvLSTM uses 12-month input sequences, resampled $64 \times 64$
LST images, two ConvLSTM layers, and a convolutional output head. It is trained
only on pre-policy data, with post-policy data held out for evaluation. For the
AlphaEarth alignment analysis, I use the mean ConvLSTM residual over the full
post-policy period 2022--2025.

\subsection{Operational AlphaEarth Change Detection}
...
\begin{itemize}
  ...
  \item paving and warming / background --- development-consistent pattern.
\end{itemize}
...
\subsection{Why AlphaEarth Improves the Project}

AlphaEarth changes the project in two ways. First, it can improve robustness.
The Landsat-only pipeline suffers from clouds, missing months, and a relatively
low revisit frequency. AlphaEarth embeddings summarize multi-sensor annual
information and are designed to be more analysis-ready. Second, AlphaEarth
addresses attribution more directly. LST residuals show where cooling happened,
but they do not show what changed on the ground. Embedding change, especially
when interpreted jointly with $\Delta$NDVI and $\Delta$LST, can help link
thermal change to surface transformation.
...
capturing spatial heterogeneity --- which pixels cooled more than the
pre-policy pattern predicts --- a signal that a spatially uniform climatology
cannot produce. The spatial residual maps show that parts of western and
southwestern Chengdu are cooler than the pre-policy reference, consistent
with spatially concentrated greening effects.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\linewidth]{fig8_convlstm_residuals.png}
  \caption{ConvLSTM residual map. Positive residuals indicate observed cooling
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\linewidth]{fig9_convlstm_timeseries.png}
  \caption{ConvLSTM predicted vs observed time series and model loss curves.}
  \label{fig:convlstm_ts}
\end{figure}

\subsection{Operational AlphaEarth Results}
...
cooling residuals in western and southwestern Chengdu are not primarily driven
...
This figure also reveals an important data-quality caveat. The annual
$\Delta$LST panel contains a large low-information region because Landsat
coverage is incomplete for many grid cells across the archive. In practice,
this makes the AlphaEarth-to-LST comparison noisier than the
AlphaEarth-to-NDVI comparison.
...
locations --- the question the current 780 m grid cannot answer. Overlaying
...
do or do not agree --- a more honest and actionable basis for causal attribution
than thermal analysis alone.
